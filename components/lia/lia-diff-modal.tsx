...
import type { LiaDiff, LiaFieldDiff } from "@/lib/lia-diff";
...
export default function LiaDiffModal({
  liaId,
  open,
  onClose,
  hasPublished,
  ...
  // Carrega lista de versões pra alimentar dropdowns
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    fetch(`/api/lia/${liaId}/versions`, { cache: "no-store" })
  ...
  const options = useMemo(() => {
    ...
    if (hasPublished) {
      out.push({ ref: "published", label: "Última publicada" });
    }
  ...
  // Busca o diff sempre que refs mudam
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    setLoading(true);
    fetch(`/api/lia/${liaId}/diff?a=${encodeURIComponent(refA)}&b=${encodeURIComponent(refB)}`, {
      cache: "no-store",
    })
      .then(async (r) => {
        if (!r.ok) {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Vê o que mudou entre 2 momentos da LIA — pergunta a pergunta, com
        ...
        <div className="flex-1 overflow-y-auto space-y-3 -mx-1 px-1">
          {loading ? (
            ...
            <>
              <div className="rounded-lg border bg-violet-50 dark:bg-violet-950/30 dark:border-violet-900 p-3 text-sm">
                <strong className="text-violet-900 dark:text-violet-200">
                  {diff.diff.stats.fieldsChanged}
                </strong>{" "}
                de{" "}
                <strong>{diff.diff.stats.totalFields}</strong> perguntas com
                alterações.
              </div>
              {diff.diff.sections
                  ...
                  <Card key={sec.key}>
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-center gap-2">
                        <span className="bg-violet-600 text-white h-6 w-6 rounded-full flex items-center justify-center text-xs font-semibold">
                          {sec.number}
                        </span>
...
function FieldDiffView({ field }: { field: LiaFieldDiff }) {
