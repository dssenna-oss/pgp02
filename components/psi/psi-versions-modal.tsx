...
interface PsiVersionsModalProps {
  psiId: string;
  publishedVersionNum: number | null;
  onClose: () => void;
}

export default function PsiVersionsModal({
  psiId,
  publishedVersionNum,
  onClose,
}: PsiVersionsModalProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const r = await fetch(`/api/psi/${psiId}/versions`, { cache: "no-store" });
        if (!r.ok) {
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Cada aprovação cria uma versão congelada (snapshot). Use "Comparar"
            no editor pra ver o que mudou entre versões.
          </DialogDescription>
        </DialogHeader>
          ...
          {loading ? (
            ...
            <Card>
              <CardContent className="py-8 text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Ainda não há versões aprovadas. Elas são criadas quando o DPO
                  aprova a PSI.
                </p>
              </CardContent>
            </Card>
            ...
            items.map((v) => {
              const isCurrent = v.version === publishedVersionNum;
              return (
                <Card
                  key={v.id}
                  className={cn(
                ...
                >
                  <CardContent className="p-4 space-y-2">
                    ...
                    {v.changeLog && (
                      <p className="text-sm whitespace-pre-wrap border-l-2 border-cyan-300 pl-3 py-0.5">
                        {v.changeLog}
                      </p>
                    )}
                  </CardContent>
