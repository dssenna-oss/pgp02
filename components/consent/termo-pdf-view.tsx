...
export default function TermoPdfView({ termId, source }: Props) {
  const [data, setData] = useState<TermResp | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/consent-terms/${termId}`, { cache: "no-store" });
        if (res.ok) {
  ...
  const typeLabel =
    CONSENT_TEMPLATE_BY_ID[data.templateType as keyof typeof CONSENT_TEMPLATE_BY_ID]
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 border-b bg-white dark:bg-gray-950 dark:border-gray-800 p-3 flex items-center justify-between gap-2 shadow-sm">
        <Button asChild variant="ghost" size="sm">
          <Link href={`/dashboard/termos-consentimento/${termId}/editar`}>
            <ArrowLeft className="h-4 w-4 mr-1" />
            Voltar ao editor
          </Link>
        </Button>
        <div className="text-xs text-gray-600 dark:text-gray-400 hidden sm:block">
          Diálogo de impressão abre automaticamente — escolha "Salvar como PDF".
        </div>
      ...
      <div className="pdf-page max-w-3xl mx-auto p-8 bg-white text-gray-900 dark:bg-white dark:text-gray-900">
        <header className="pdf-section pb-6 border-b border-gray-300 mb-6">
          <p className="text-xs uppercase tracking-widest text-violet-700 font-semibold">
            Termo de Consentimento · {typeLabel}
          </p>
        ...
        {/* Linha de assinatura — útil pra modo físico */}
        <div className="mt-12 pt-8 border-t border-gray-300">
          <p className="text-sm mb-6">
            <strong>Local e data:</strong> _________________________________________________________
          </p>
          <div className="grid grid-cols-1 gap-8">
            <div>
              <p className="text-sm mb-1"><strong>Nome do titular:</strong> ________________________________________</p>
              <p className="text-sm mb-1"><strong>CPF:</strong> ____________________________</p>
              <p className="text-sm mt-8 pt-4 border-t border-gray-400 w-72">
                Assinatura
              </p>
            </div>
          </div>
      ...
      <style jsx global>{`
        ...
        .policy-article h1 { font-size: 1.875rem; font-weight: 700; margin: 2rem 0 1rem; }
        .policy-article h2 { font-size: 1.5rem; font-weight: 600; margin: 2rem 0 0.75rem; }
