...
export default function SugestaoCartaContent() {
  ...
  // Pré-popula o domain com o que está em /api/company/institutional-domain
  useEffect(() => {
    let abort = false;
    (async () => {
      try {
        const res = await fetch("/api/company/institutional-domain");
        if (!res.ok || abort) return;
        const json = await res.json();
        if (json?.institutionalDomain && !abort) {
          setDomain(json.institutionalDomain);
          setDomainPrefilled(true);
        }
      } catch {
        // silencioso
      }
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      ...
      {!result && !loading && (
        <div className="bg-white dark:bg-gray-800 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-8 text-center">
          ...
          <p className="text-xs text-gray-500">
            Cole um domínio e clique em "Sugerir processos" — a IA varre as
            páginas públicas e devolve uma lista pra você revisar.
          </p>
        </div>
      )}
