...
>
> **🚨 Incidente 2026-05-11 (resolvido)**: apliquei migration Etapa 28 via `prisma db execute` mas DATABASE_URL local apontava pra `postgres@localhost` (dev), não pro Neon de prod. Build do Vercel deployou Prisma Client com a coluna nova mas Neon prod não tinha → 500 em todos endpoints com auth. Recovery: SQL `ALTER TABLE companies ADD COLUMN IF NOT EXISTS institutionalDomain TEXT` rodado pelo user no Neon SQL Editor, prod voltou. Lição em `feedback_migration_no_banco_errado.md`.
>
> **🟡 PENDENTE: FIRECRAWL_API_KEY não chega em runtime mesmo cadastrada no projeto correto.** User tem 2 projetos Vercel (`pgp02` vazio + `pgp` que serve `lgpd-pgp.vercel.app`). Após cadastrar a chave no projeto certo (`pgp`) com Sensitive ON em Production+Preview, o `process.env.FIRECRAWL_API_KEY` **continua chegando vazio** (`keyLen: 0`) enquanto outras Sensitive (BREVO/GOOGLE) chegam normais. Endpoint debug em `app/api/admin/env-check/route.ts` pra diagnose. **Workaround imediato**: opção "Outras URLs (manual)" do modal de Pré-preencher funciona normal — user cola URL do site direto e Firecrawl scrape funciona via /v1/scrape (não usa /v1/map). **Próximas tentativas em sessão futura**: (1) confirmar que Redeploy completou de fato olhando Deployments → Status Ready; (2) editar a var, desligar Sensitive, salvar, ver se passa a chegar; (3) deletar var e recadastrar com nome ligeiramente diferente (FIRECRAWL_KEY_PROD?); (4) abrir ticket Vercel se persistir. Endpoint `/api/admin/env-check` deve ser **REMOVIDO** assim que mistério for resolvido.
>
