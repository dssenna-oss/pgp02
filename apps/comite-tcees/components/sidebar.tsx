  ...
  return (
    <>
      ...
      <nav className="flex-1 overflow-auto p-2.5">
        <div className="px-2.5 pt-2 pb-1.5 text-[10px] font-bold uppercase tracking-wider text-blue-100">
        ...
        {[
          ...
          { href: "/dashboard/checklist-monitoramento", icon: ListTodo, label: "Checklist Monitoramento", fase: "Fase 7" },
          { href: "/dashboard/auditoria", icon: FileSearch, label: "Auditoria Interna", fase: "Fase 7" },
          ...
          return (
            <Link
              ...
              className={cn(
                "flex items-center gap-2.5 px-2.5 py-2.5 rounded-lg text-[13.5px] mb-0.5 transition-colors",
                active ? "bg-navy text-white font-semibold shadow-sm" : "text-slate-100 hover:bg-white/10 hover:text-white",
              )}
            >
              <Icon className="w-[18px] h-[18px] shrink-0" /> {label} <span className={cn("text-[10px] font-normal", active ? "text-blue-100" : "text-blue-200")}>· {fase}</span>
            </Link>
          );
        })}
      </nav>
      <div className="px-4 py-3 text-[11px] text-blue-100/80 border-t border-white/10">
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Desktop */}
      <aside className="hidden lg:flex w-[248px] shrink-0 flex-col bg-tcees text-slate-100 sticky top-0 h-screen">
      ...
      <button
        ...
        className="lg:hidden fixed top-3 left-3 z-30 bg-tcees text-white p-2 rounded-lg shadow"
      >
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative w-[260px] flex flex-col bg-tcees text-slate-100 h-full">
            <button
