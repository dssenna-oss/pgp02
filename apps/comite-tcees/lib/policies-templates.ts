...
export function applyPlaceholders(
  ...
  const map: Record<string, string> = {
    ...
    // Placeholders que no app principal vêm do agregador do Inventário.
    // No app do Comitê (mono), o Encarregado preenche manualmente no editor.
    "{{nome}}": c.companyName,
    "{{categorias_dados}}": "[categorias de dados — detalhar a partir do Inventário]",
    "{{matriz_tratamento}}": "[matriz de tratamentos — detalhar a partir do Inventário]",
    "{{tipos_compartilhamento}}": "[compartilhamentos — detalhar a partir do Inventário]",
  };
  let out = template;
